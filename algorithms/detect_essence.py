...
class EssenceFinder(BaseAlgorithm[EssenceFinderResult]):
    ...
    def parse(self, product_name: ProductNameToken) -> EssenceFinderResult:
        res = []
        if product_name is not None:
            for context, token in TokenSeq.from_string(product_name.value).iter_with_context():
                if can_be_essence(context, token):
                    res.append(EssenceFinderResult(None, token.value.capitalize()))
...
class SelectEssence(BaseSelector[EssenceFinderResult]):
    def select(self, input: list[EssenceFinderResult]) -> EssenceFinderResult:
        return input[0] if input else EssenceFinderResult(None, None)
